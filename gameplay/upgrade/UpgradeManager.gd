...
static var add_backpack_level_limits : Array[int] = [1, 2, 3, 5]
static var add_column_level_limits : Array[int] = [1, 2, 3, 5]
...
static var _count_times_bought : Array[Callable] = [
	func () : return min(add_column_costs.size() - 1, Database.backpacks_purchased),
	func () : return min(add_column_costs.size() - 1, Database.columns_purchased),
	func () : return min(increase_capacity_costs.size() - 1, Database.capacity_increases_purchased),
	func () : return min(change_column_region_costs.size() - 1, Database.active_region_columns),
	func () : return min(change_column_customer_costs.size() - 1, Database.active_customer_columns),
	func () : return 0,
	func () : return min(remodel_costs.size() - 1, Database.shop_level),
]
